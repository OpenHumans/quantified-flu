...
class RespectfulRequester:
    def __init__(self):
        ...
        try:
            ...
            raise RequestsRespectfulRedisError(
                "Could not establish a connection to the provided Redis server"
            )
    ...
    def request(self, request_func, realm=None, realms=None, wait=False):
        if realm is not None:
            warnings.warn(
                "'realm' kwarg will be removed in favor of providing a 'realms' list starting in 0.3.0",
                DeprecationWarning,
            )
            realms = [realm]
        ...
        for r in realms:
            if r not in registered_realms:
                raise RequestsRespectfulError(
                    "Realm '%s' hasn't been registered" % realm
                )
    ...
    def fetch_registered_realms(self):
        return list(
            map(
                ...
                self.redis.smembers("%s:REALMS" % self.redis_prefix),
            )
        )
    ...
    def register_realm(self, realm, max_requests, timespan):
        ...
        if not self.redis.hexists(redis_key, "max_requests"):
            self.redis.hmset(
                redis_key, {"max_requests": max_requests, "timespan": timespan}
            )
    ...
    def configure(cls, **kwargs):
        if "redis" in kwargs:
            ...
            if len(missing_redis_keys):
                raise RequestsRespectfulConfigError(
                    "'%s' %s missing from the 'redis' configuration key"
                    % (
        ...
        if "safety_threshold" in kwargs:
            if (
                type(kwargs["safety_threshold"]) != int
                or kwargs["safety_threshold"] < 0
            ):
                raise RequestsRespectfulConfigError(
                    "'safety_threshold' key must be a positive integer"
                )
        ...
        if "requests_module_name" in kwargs:
            if type(kwargs["requests_module_name"]) != str:
                raise RequestsRespectfulConfigError(
                    "'requests_module_name' key must be string"
                )
    ...
    def _perform_request(self, request_func, realms=None):
        ...
        if not len(rate_limited_realms):
            ...
            raise RequestsRespectfulRateLimitedError(
                "Currently rate-limited on Realm(s): %s"
                % ", ".join(rate_limited_realms)
            )
    ...
    def _can_perform_request(self, realm):
        return self._requests_in_timespan(realm) < (
            self.realm_max_requests(realm) - config["safety_threshold"]
        )
    ...
    def _requests_proxy(self, method, *args, **kwargs):
        ...
        return self.request(
            lambda: getattr(requests, method)(*args, **kwargs), realms=realms, wait=wait
        )
    ...
    def _validate_request_func(request_func):
        ...
        if not post_lambda_string.startswith(
            config["requests_module_name"]
        ) and not post_lambda_string.startswith("getattr(requests"):
            raise RequestsRespectfulError(
                "The request lambda can only contain a requests function call"
            )
